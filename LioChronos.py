...
# ==========================================
# 1. CONFIGURAÇÃO VISUAL E FUNÇÕES BASE
# ==========================================
...
with st.sidebar:
    ...
    st.markdown("---")
    st.header("📂 Gerir Dados")
    
    st.subheader("1. Guardar Trabalho")
    st.info("Descarregue o ficheiro para não perder as turmas e professores registados.")
    ...
    st.markdown("---")
    st.subheader("2. Restaurar Dados")
    arquivo_importado = st.file_uploader("Carregar JSON de backup", type=["json"])
